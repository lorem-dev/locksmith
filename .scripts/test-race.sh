#!/usr/bin/env bash
# Run 'go test -race ./...' for every module in the Go workspace.
set -e
...
TOTAL=$((PASS + FAIL))
if [ "$FAIL" -eq 0 ]; then
  printf '\033[1m\033[32m=== %d/%d modules passed (race) ===\033[0m\n' "$PASS" "$TOTAL"
else
  printf '\033[1m\033[31m=== %d/%d modules passed (race) ===\033[0m\n' "$PASS" "$TOTAL"
  exit 1
fi
