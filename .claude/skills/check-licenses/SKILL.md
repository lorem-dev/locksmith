...
1. **License SPDX identifier** - Use the `WebFetch` tool to fetch
   `https://pkg.go.dev/<module>` and read the "Licenses" field from the page.
...
2. **License URL** - direct link to the `LICENSE` file in the upstream
   ...
   For pseudo-versions (format: `v0.0.0-YYYYMMDDHHMMSS-<commit-sha>`), use
   the 12-character commit SHA suffix as the ref:
   `https://github.com/<org>/<repo>/blob/<12-char-sha>/LICENSE`.
...
separator line). If the section already exists (incremental mode), edit the
entries in place - do not add another separator.
...
If `## GPG Signing` is not present in `CONTRIBUTING.md`, append the section
